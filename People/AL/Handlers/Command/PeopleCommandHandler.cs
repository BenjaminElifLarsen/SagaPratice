...
internal sealed class PeopleCommandHandler : IPeopleCommandHandler
{
    ...
    public Result Handle(ChangePersonalInformationFromUser command)
    {
        ...
        if (!flag)
        { //have event
            entity.AddDomainEvent(new PersonPersonalInformationChangedFailed(entity, PersonErrorConversion.Convert(flag), entity.OldEventsDesign.Count(),  command.CorrelationId, command.CommandId));
    ...
    public Result Handle(RecogniseGender command)
    {
        ...
        result.Data.AddDomainEvent(new GenderRecognisedSucceeded(result.Data, result.Data.OldEventsDesign.Count(), command.CorrelationId, command.CommandId));
